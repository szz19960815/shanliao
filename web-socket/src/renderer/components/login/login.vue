<template>
  <div id="login" class="main">
    <!-- 这是登录页 -->
    <div
      style="display: flex;flex-direction: column;background: white;border-radius: 15px;overflow: hidden;"
    >
...
export default {
    ...
    login(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.$http
            .post("/users/login", config)
            .then(res => {
              if (res.data.code == 1) {
                // this.$socket.emit("sureConnect", {
  ...
  beforeMount() {
    if (localStorage.getItem("username") != undefined) {
      this.loginForm.user = JSON.parse(
